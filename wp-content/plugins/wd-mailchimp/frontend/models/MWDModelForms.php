...
#minimize-form".$form_id." {
	top: ".$form_theme['CBPTop'].";
	".$for_mini.": ".(2 * intval($form_theme['CBP'.ucfirst($for_mini)]) + intval($form_theme['CBPFontSize']))."px;
	bottom: ".$form_theme['CBPBottom'].";
}";
...
#mwd-form".$form_id." .check-rad.mwd-right input[type='radio'] {
    ...
    $api_key = get_option('mwd_api_key', '');
		try {
      $mchlists = MWD_Library::mailchimp_curl_connect( '/lists/', 'GET', $api_key );
		} catch (Exception $e) {
			return;
		}
    ...
    $merge_fields = json_decode(json_encode($merge_vars));
    if($fparams['email_field'] && $subscribe_to_mailchimp && $list_ids){
			...
			foreach($list_ids as $list_id) {
				...
				if($subscribe_action == 1) {	
					$mwd_merge_vars = (array) MWD_Library::mailchimp_curl_connect( '/lists/'.$list_id.'/merge-fields/', 'GET', $api_key );

					$mwd_merge_vars = $mwd_merge_vars['merge_fields'];
					$req_merges = array();
					foreach($mwd_merge_vars as $mwd_merge_var){
						if($mwd_merge_var->required){
					...
					if($use_mailchimp_email == 1) {
						$_params = array(
							'email_address' => sanitize_email($merge_vars['EMAIL']),
							'merge_fields' => $merge_fields,
							'email_type' => $email_type,
              'status' => 'subscribed',
						);
						try {
							$response_data = MWD_Library::mailchimp_curl_connect( '/lists/'.$list_id.'/members', 'POST', $api_key, $_params );
						...
						if($double_optin != 1) {
							$_params = array( 
								'email_address' => array( 'email' => sanitize_email($merge_vars['EMAIL'])),
                ...
                $response_data = MWD_Library::mailchimp_curl_connect( '/lists/'.$list_id.'/members', 'POST', $api_key, $_params );
							...
							if($merge_vars['EMAIL'] === NULL){
								...
								$emails_list = array();
								$subscribers_list = MWD_Library::mailchimp_curl_connect( '/lists/'.$list_id, 'GET', $api_key );
                  ...
                  $updating = true;
                    ...
                    'status_if_new' => 'subscribed',
									...
									try {
                    $response_data = MWD_Library::mailchimp_curl_connect( '/lists/'.$list_id.'/members', 'PUT', $api_key, $_params );
										if($form->success_message) {
											$_SESSION['mwd_message_after_submit' . $id] = array('msg' => $this->get_message($id, 'update_message', $response_data), 'type' => 'success');
										}
          ...
          $sub_id = md5($merge_vars['EMAIL']);
					try {
            $response_data = MWD_Library::mailchimp_curl_connect( '/lists/'.$list_id.'/members/'.$sub_id, 'PUT', $api_key);
    ...
    $api_key = get_option('mwd_api_key', '');
		...
		$profile_info = (array) MWD_Library::mailchimp_curl_connect( '/lists/'.$row->list_id.'/members', 'GET', $api_key );
		$mwd_list = (array) MWD_Library::mailchimp_curl_connect( '/lists/'.$row->list_id, 'GET', $api_key );
		$list_id = $mwd_list['id'];
		$list_name = $mwd_list['name'];
		$list_subject = $mwd_list['campaign_defaults']->subject;
		$list_from_email = $mwd_list['campaign_defaults']->from_email;
		$list_from_name = $mwd_list['campaign_defaults']->from_name;
		$list_owner_email = $profile_info['members'][0]->email_address;
		$user_mailchimp_id = $row->subscribe_action == 1 && isset($response_data->unique_email_id) ? $response_data->unique_email_id : '';
    ...
    $api_key = get_option('mwd_api_key', '');
		try {
      $mchlists = (array) MWD_Library::mailchimp_curl_connect( '/lists/'.$list_ids, 'GET', $api_key );
		} catch (Exception $e) {
		...
		if ( is_array($mchlists)) {
			foreach ( $mchlists['data'] as $list ) {
				try {
          $interest_groups = (array) MWD_Library::mailchimp_curl_connect( '/lists/'.$list_ids.'/interest-categories', 'GET', $api_key );
				} catch( Exception $e ) {
				}
			}
		}
