...
class MWDControllerHelper {
  private $api_key;
    ...
    $this->api_key = MWD_Library::get('apikey');
    $isvalid_apikey = $model->mwd_validate_api( $this->api_key, '/ping/' );
    ...
    $page_data['profile_info'] = $account_details;
	...
	public function lists() {
		...
		$isvalid_apikey = $model->mwd_validate_api($apikey, '/ping/');
		if(get_option('mwd_api_validation') == 'invalid_apikey') {
			echo "<b style='color:red'>".$isvalid_apikey."</b>\n";
			die();
		}
  ...
  public function merge_variables($list_id) {
    $apikey = get_option('mwd_api_key', '');
    $merge_variables_data = MWD_Library::mailchimp_curl_connect( '/lists/'.$list_id.'/merge-fields', 'GET', $apikey );

    $merge_variables = $merge_variables_data['merge_fields'];
