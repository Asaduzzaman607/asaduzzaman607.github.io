...
class MWDViewManage_mwd {
	...
	public function display() {
		...
		if(get_option('mwd_api_validation') == 'valid_apikey') {
			if(false === ($mwd_lists = !get_transient('mwd-list-info')) || empty($mwd_lists = get_transient('mwd-list-info'))) {
    ...
    $mwd_account_details = (array) $mwd_account_details;
			...
			<div class="row">
										...
										<tr>
											...
											<td colspan=2 style="padding: 0; border: 0;">
												<table class="mwd-merge-vars-table" cellspacing="0">
													...
													<tbody>
													<?php
                          foreach($mwd_list->interest_groups->categories as $interest_group):
                            ?>
														<tr>
				...
				<div class="col-md-4 col-sm-12 col-xs-12">	
					<div class="mwd-sidebar mwd-account-info">
						...
						<img src="<?php echo $mwd_account_details['avatar_url']; ?>" />
						<div class="profile-general">
							<span class="mwd-profile-name"><?php echo $mwd_account_details['first_name'].'<br /> '.$mwd_account_details['last_name']; ?></span><?php echo ' ('.$mwd_account_details['role'].')'; ?>
							<p><?php echo $mwd_account_details['username']; ?></p>
							<p><?php echo $mwd_account_details['email']; ?></p>
						</div>
						<div>
							<div class="mwd-row">
							...
							<div class="mwd-row">
								...
								<div class="mwd-value">
									<?php echo isset($mwd_account_details['account_industry']) ? $mwd_account_details['account_industry'] : ''; ?>
								</div>
							</div>
							<div class="mwd-row">
								...
								<div class="mwd-value">
									<?php echo isset($mwd_account_details['member_since']) ? date("Y-m-d H:i:s", strtotime($mwd_account_details['member_since'])) : ''; ?>
								</div>
							</div>
							<div class="mwd-row">
								...
								<div class="mwd-value">
									<?php echo isset($mwd_account_details['pricing_plan_type']) ? $mwd_account_details['pricing_plan_type'] : ''; ?>
								</div>
							</div>
							<div class="mwd-row">
		...
		<script>
				...
				if(jQuery('#mwd_mailchimp_apikey').val()) {
					if(!jQuery('#mwd_mailchimp_apikey').attr('disabled'))
						...
						success:function(data) {
							...
							jQuery('.mwd-mailchimp-loaded').html('2/2');
							jQuery('.mwd-mailchimp-loaded').css('width','50%');
			...
			jQuery( document ).ready(function() {
				...
				jQuery('.mwd-create-form').on('click', function() {
					window.location = '<?php echo add_query_arg(array('task' => 'add', 'nonce_mwd' => wp_create_nonce('nonce_mwd')), admin_url('admin.php?page=manage_forms')); ?>&list_id='+jQuery('#mwd_mailchimp_list').val();
				});
