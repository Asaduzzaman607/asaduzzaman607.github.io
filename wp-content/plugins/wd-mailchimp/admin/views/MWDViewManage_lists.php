...
class MWDViewManage_lists {
	...
	public function display() {
		...
		$mwd_lists = (array) $this->model->get_lists_search();
		...
		<div class="mwd-mailchimp mwd-wrap">
			<form onkeypress="mwd_doNothing(event)" id="manage_lists" method="post" action="admin.php?page=manage_lists">
				...
				<table class="widefat fixed pages">
					...
					<tbody id="tbody_arr">
						<?php
						if ($mwd_lists['total_items']) {
							foreach ($mwd_lists['lists'] as $index => $mwd_list) {
                      ...
                      <?php echo $mwd_list->name; ?>
									...
									<td>
										<?php
                    $d=strtotime($mwd_list->date_created);
	...
	public function view($list_id) {
		if(get_option('mwd_api_validation') == 'valid_apikey' ) {
			...
			if(false === ($list_data = get_transient('mwd-list-data'))) {
				$list_data = (array) $this->model->get_list_data( $list_id );
		...
		$subscribers_list = (array) $this->model->get_subscribers_list_search( $list_id );
		$list = (array) $list_data['list'];
		$merge_vars = $list_data['merge_vars'];
		$interest_groups = (array) $list_data['interest_groups'];
		$segments = (array) $list_data['segments'];
    ...
    $search_value = ((isset($_POST['search_value'])) ? esc_html($_POST['search_value']) : '');
		...
		$order_class = 'manage-column column-title sorted ' . $asc_or_desc;
		$users_count = $subscribers_list['total_items'];
				...
				<div class="col-md-8 col-sm-6">
					<form id="mwd_subscribers" method="post">
						...
						<table class="widefat fixed pages">
							...
							<tbody id="tbody_arr">
								<?php
								if ($subscribers_list['total_items']) {
									foreach ($subscribers_list['members'] as $index => $subscriber) {
                    $subscriber = (array) $subscriber;
										$alternate = (!isset($alternate) || $alternate == '') ? 'class="alternate"' : '';
										if(!empty($subscriber['email_client'])) {
											$client_name = $subscriber['email_client']['name'];
                    ...
                    ?>
										<tr id="tr_<?php echo 20 * ($page_nav['limit'] - 1) + $index + 1; ?>" <?php echo $alternate; ?>>
											<td class="table_small_col"><?php echo 20 * ($page_nav['limit'] - 1) + $index + 1; ?></td>
											<td>
												<a href="<?php echo add_query_arg(array('list_id' => $list['id'],'sub_id'=>md5($subscriber['email_address'])), admin_url('admin.php?page=manage_lists&task=subscriber_info')); ?>">
                          <?php echo $subscriber['email_address']; ?>
											...
											<td>
												<?php echo date('Y-m-d H:i:s',strtotime($subscriber['last_changed'])); ?>
											</td>
											<td class="text-center">
                        <?php
                        $email = md5($subscriber['email_address']);
                        ?>
												<a href="<?php echo add_query_arg( array('email' => $email, 'list_id' => $list['id'], 'nonce_mwd' => wp_create_nonce('nonce_mwd')), admin_url('admin.php?page=manage_lists&task=unsubscribe') ) ?>" style="display:inline-block;">
					...
					<div class="mwd-sidebar mwd-fields-info">
						...
						<div class="panel">
							<div class="panel-heading text-center">
								<?php echo intval($list['stats']->merge_field_count+1); ?> Fields
							</div>
				...
				<div class="col-md-4 col-sm-6">
					<div class="mwd-sidebar mwd-list-info">
						...
						<div class="list-general">
							...
							<div>
								...
								<div class="mwd-value mwd-arrow mwd-arrow-right">
									<input type="text" value="<?php echo sanitize_email($list['campaign_defaults']->from_email); ?>" readonly onclick="jQuery(this).select();" />
								</div>
							</div>
							<div>
								<div class="mwd-key">
								...
								<div class="mwd-value mwd-arrow mwd-arrow-right">
									<?php echo $list['campaign_defaults']->from_name; ?>
								</div>
							</div>
							<div>
								<div class="mwd-key">
								...
								<div class="mwd-value mwd-arrow mwd-arrow-right">
									<?php echo intval($list['stats']->merge_field_count+1); ?>
								</div>
							</div>
							<div>
								<div class="mwd-key">
					...
					<div class="mwd-sidebar mwd-interest-groups-info">
						...
						<div class="panel">
							<div class="panel-heading text-center">
								<?php 
									if(is_array($interest_groups['categories'])) {
										echo intval(count( $interest_groups['categories'])).' Merge Variables';
									} 
								?>
							</div>
							<div class="panel-body mwd-arrow mwd-arrow-top">
								<?php if(is_array($interest_groups['categories'])) { ?>
									<ul>
										<?php
                    foreach($interest_groups['categories'] as $interest_group) { ?>
											<li>
												<span><?php echo $interest_group->title; ?></span><span>&nbsp;&nbsp;(<?php echo $interest_group->groups[0]->subscriber_count; ?>)</span>
											</li>
										<?php }	?>
									</ul>
								<?php } else {
									echo $interest_groups['categories'];
								} ?></div>
						</div>
					</div>
					<br /><br />
					<div class="mwd-sidebar mwd-segments-info">
						...
						<div class="panel">
							<div class="panel-heading text-center">
								<?php
									if(is_array($segments['segments']) && count( $segments['segments'] ) > 0) {
										echo intval(count($segments['segments'])).' Segment';
									}	
								?>
							</div>
							<div class="panel-body mwd-arrow mwd-arrow-top">
								<?php if(is_array($segments['segments'])) {
									if(isset( $segments['segments'][0]->type ) && $segments['segments'][0]->type == "saved" && count( $segments['segments'] ) > 0) { ?>
									<ul>
										<?php 
										foreach($segments['segments'] as $segment) {
											...
											<div class="mwd-segment-conditions hide">
												<ul>
													<?php
													foreach( $segment->options->conditions as $condition ) {
    ...
    $profile_data = (array) $profile_data;
		if(isset($profile_data)) {
			$user_email = sanitize_email($profile_data['email_address']);
			$list_name = $profile_data['list_name'];
		} else {
			$list_name = '';
			$user_email = '';
		}
		...
		<div class="mwd-mailchimp container-fluid">
          ...
          $other_lists = (isset($profile_data) && ! empty($profile_data)) ? $profile_data : array();
					$merge_variables_user = ($profile_data && ! empty( $profile_data)) ? $profile_data : array();

					$additional_lists = array();
					if( isset( $other_lists ) && is_array($other_lists) && count( $other_lists ) >= 1 ) {
						foreach( $other_lists as $list ) {
							if( $list == 'subscribed' ) {
								$list_data = MWD_Library::mailchimp_curl_connect( '/lists/'.$other_lists['list_id'], 'GET', $api_key);
								if( $list_data && isset( $list_data ) ) {
									$additional_lists[$list_data->id] = $list_data->name;
								}
							}
						}
					}
					if( isset( $merge_variables_user ) && (is_array($merge_variables_user) || is_object($merge_variables_user)) && count( (array) $merge_variables_user ) >= 1 ) {
            $merge_variables = (array) MWD_Library::mailchimp_curl_connect( '/lists/'.$list_id.'/merge-fields', 'GET', $api_key );
						$merge_variable_fields = array();
						if( $merge_variables ) {
              $merge_variables_user = (array) $merge_variables_user;
              $merge_variables_user['merge_fields'] = (array) $merge_variables_user['merge_fields'];

							foreach( $merge_variables['merge_fields'] as $merge_variable ) {
                $merge_variable = (array) $merge_variable;
                $merge_variable_fields[$merge_variable['name']] = ( isset( $merge_variables_user['merge_fields'][$merge_variable['tag']] ) ) ? $merge_variables_user['merge_fields'][$merge_variable['tag']] : '';
							}
						}
					...
					$last_changed = strtotime( $profile_data['last_changed'] );
					$user_language = ( $profile_data['language'] && $profile_data['language'] != '' ) ? $profile_data['language'] : '';
					
					$unsubscribe_url = esc_url_raw(add_query_arg( array('list_id' => $list_id, 'nonce_mwd' => wp_create_nonce('nonce_mwd')), admin_url('admin.php?page=manage_lists&task=unsubscribe')));
					?>
					<div class="col-md-8 col-sm-10 col-xs-12">
						<div class="mwd-subscriber-details col-md-12">
							...
							<div class="col-md-9 col-xs-8">
								...
								<p>Subscribed: <?php echo date('F jS, Y h:i a', strtotime($profile_data['last_changed'])); ?></p>
								<?php
                if(isset( $profile_data['location']) && ! empty( $profile_data['location']) && $profile_data['location']->latitude != 0 && $profile_data['location']->longitude) {
								  ?>
									<?php if(isset($profile_data['location']->latitude) && isset($profile_data['location']->longitude)){ ?>
										<p>
											Location: <?php echo $this->mwd_get_geocode( $profile_data['location']->latitude, $$profile_data['location']->longitude ); ?>
										</p>
									<?php } else { ?>
										<p>
											Location: <?php echo $profile_data['location']->dstoff . ', ' . $profile_data['location']->gmtoff; ?>
										</p>
									<?php 
									}
								}
							...
							<div class="col-md-12">
								...
								<div class="mwd-subscriber-content merge-variables-tab row" style="margin: -3px -1px 0 !important;">
									<div class="col-md-12">
										...
										if(isset( $profile_data['unique_email_id'] ) && $profile_data['unique_email_id'] != '') { ?>
											<div class="mwd-row">
												<div class="mwd-key merge-var-label">
													User ID
												</div>
												<div class="mwd-value merge-var-value">
													<span><em><?php echo $profile_data['unique_email_id']; ?></em></span>
												</div>
											</div>
											<?php
										}
