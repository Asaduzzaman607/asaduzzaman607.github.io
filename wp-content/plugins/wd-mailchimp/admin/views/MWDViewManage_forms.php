...
class MWDViewManage_forms {
	...
	public function display() {
		...
		$lists = (array) $this->model->get_lists();
		...
		$lists = array_combine( array_map(function($k){ return $k->id; }, $lists['lists']), array_map(function($v){ return $v->name; }, $lists['lists']));
			...
			<div class="panel form-content">
				<div class="panel-content">
					<div class="mwd-2col">
						...
						<div class="mwd-row list-fields mailchimp-field">
							<?php if($list_ids) {
								...
								foreach( $list_ids as $lkey => $listID ){
									...
									$intGroups = $listData->interest_groups->categories;
									if( $intGroups ){
										$groupToAddAfter .=  $lkey == 0 ? '<br /><br /><div class="mini-label intgroup">Interest groups</div>' : '';
										$m = (is_array($intGroups) || is_object($intGroups)) ? count($intGroups) : '';
										for($k=0; $k<$m; $k++){
											if(in_array($intGroups[$k]->title, $groupNameExist))
												$merges[$l]->name .= '(1)';
                      ...
                      $form_field = isset($intGroups[$k]->form_field) ? $intGroups[$k]->form_field : "";
											$groupToAddAfter .= '<button data-fieldType="'.$form_field.'" data-id="'.$intGroups[$k]->id.'" class="'.$informclass.'" onclick="add_mailchimp_group(this); return false;">'.$intGroups[$k]->title.'</button>';

											$groupNameExist[] = $intGroups[$k]->title;
										}
									}
								}
