...
class MWDModelSubmissions_mwd {
	...
	public function blocked_ips() {
		global $wpdb;
		$ips = $wpdb->get_col('SELECT ip FROM ' . $wpdb->prefix . 'mwd_forms_blocked');
		return $ips;
	}

	public function get_form_titles() {
		global $wpdb;
		$query = "SELECT id, title FROM " . $wpdb->prefix . "mwd_forms order by title";
	...
	public function get_statistics($form_id) {
		global $wpdb;
		$statistics = array();
		$query = $wpdb->prepare('SELECT count(distinct group_id) FROM ' . $wpdb->prefix . 'mwd_forms_submits WHERE form_id ="%d"', $form_id);
		$statistics["total_entries"] = $wpdb->get_var($query);
		$query = $wpdb->prepare('SELECT `views` FROM ' . $wpdb->prefix . 'mwd_forms_views WHERE form_id="%d"', $form_id);
	...
	public function get_labels_parameters($form_id) {
		...
		if ($form_id) {
			...
			$query = "SELECT distinct element_label FROM " . $wpdb->prefix . "mwd_forms_submits WHERE ". $where;
			$results = $wpdb->get_results($query);
			for ($i = 0; $i < count($results); $i++) {
				array_push($labels, $results[$i]->element_label);
			}
			$form = $wpdb->get_row($wpdb->prepare("SELECT * FROM " . $wpdb->prefix . "mwd_forms WHERE id='%d'", $form_id));
		...
		switch (count($join_query)) {
			case 0:
				$join = 'SELECT distinct group_id FROM ' . $wpdb->prefix . 'mwd_forms_submits WHERE '. $where;
			break;
			case 1:
				if ($join_query[0] == 'sort') {
					$join = 'SELECT group_id FROM ' . $wpdb->prefix . 'mwd_forms_submits WHERE ' . $where . ' AND element_label="' . $join_where[0]['label'] . '" ';
					$join_count	= 'SELECT count(group_id) FROM ' . $wpdb->prefix . 'mwd_forms_submits WHERE form_id="' . $form_id . '" AND element_label="' . $join_where[0]['label'] . '" ';
					$orderby =	' ORDER BY `element_value` ' . $asc_or_desc;
				}
				else {
					$join = 'SELECT group_id FROM ' . $wpdb->prefix . 'mwd_forms_submits WHERE element_label="' . $join_where[0]['label'] . '" AND  element_value LIKE "%' . $join_where[0]['search'] . '%" AND ' . $where;
				}
			break;
			default:
				$join = 'SELECT t.group_id FROM (SELECT ' . $cols . '  FROM ' . $wpdb->prefix . 'mwd_forms_submits WHERE ' . $where . ' AND element_label="' . $join_where[0]['label'] . '" AND  element_value LIKE "%' . $join_where[0]['search'] . '%" ) as t ';
				for ($key = 1; $key < count($join_query); $key++) {
					if ($join_query[$key] == 'sort') {
						$join .= 'LEFT JOIN (SELECT group_id as group_id' . $key . ', element_value   FROM ' . $wpdb->prefix . 'mwd_forms_submits WHERE ' . $where . ' AND element_label="' . $join_where[$key]['label'] . '") as t' . $key . ' ON t' . $key . '.group_id' . $key . '=t.group_id ';
						$orderby = ' ORDER BY t' . $key . '.`element_value` ' . $asc_or_desc . '';
					}
					else {
						$join .= 'INNER JOIN (SELECT group_id as group_id' . $key . ' FROM ' . $wpdb->prefix . 'mwd_forms_submits WHERE '.$where.' AND element_label="' . $join_where[$key]['label'] . '" AND  element_value LIKE "%' . $join_where[$key]['search'] . '%" ) as t' . $key . ' ON t' . $key . '.group_id' . $key . '=t.group_id ';
					}
				}
				break;
		}
		...
		$query = 'SELECT * FROM ' . $wpdb->prefix . 'mwd_forms_submits ' . $where2;
	...
	public function statistic_for_radio($where_choices, $sorted_label_id) {
		...
		$query = "SELECT element_value FROM " . $wpdb->prefix . "mwd_forms_submits WHERE " . $where_choices . " AND element_label='" . $sorted_label_id . "'";
	...
	public function get_data_of_group_id($id) {
		global $wpdb;
		$query = "SELECT * FROM " . $wpdb->prefix . "mwd_forms_submits WHERE group_id=" . $id;
		$rows = $wpdb->get_results($query);
		$form = $wpdb->get_row("SELECT * FROM " . $wpdb->prefix . "mwd_forms WHERE id=" . $rows[0]->form_id);
		$params = array();
		$label_id = array();
		...
		$theme_id = $wpdb->get_var("SELECT theme FROM " . $wpdb->prefix . "mwd_forms where id = ".$rows[0]->form_id);
	...
	public function view_for_star_rating($element_value, $element_label) {
		...
		for ($j = 0; $j < $new_filename[1]; $j++) {
			$stars .= '<img id="' . $element_label . '_star_' . $j . '" src="' . MWD_URL . '/images/star_' . $new_filename[2] . '.png?ver='. get_option("mwd_version").'" /> ';
		}
		for ($k = $new_filename[1]; $k < $new_filename[0]; $k++) {
			$stars .= '<img id="' . $element_label . '_star_' . $k . '" src="' . MWD_URL . '/images/star.png?ver='. get_option("mwd_version").'" /> ';
		}
	...
	public function images_for_star_rating($element_value, $label_id) {
		...
		for ($j = 0; $j < $stars_value[1]; $j++) {
			$edit_stars .= '<img id="'.$label_id.'_star_'.$j.'" onclick="edit_star_rating('.$j.','.$label_id.')" src="' . MWD_URL . '/images/star_'.$stars_value[2].'.png?ver='. get_option("mwd_version").'" /> ';
		}
		for( $k=$stars_value[1];$k<$stars_value[0];$k++) {
			$edit_stars .= '<img id="'.$label_id.'_star_'.$k.'" onclick="edit_star_rating('.$k.','.$label_id.')" src="' . MWD_URL . '/images/star.png?ver='. get_option("mwd_version").'" /> ';
		}
	...
	public function get_subs_count($form_id) {
		global $wpdb;
		$query = $wpdb->prepare("SELECT distinct group_id FROM " . $wpdb->prefix . "mwd_forms_submits where form_id=%d", $form_id);
	...
	public function get_from_label_order($form_id) {
		global $wpdb;
		$label_order = $wpdb->get_var($wpdb->prepare('SELECT `label_order` FROM `' . $wpdb->prefix . 'mwd_forms` WHERE id="%d"', $form_id));
		return $label_order;
	}

	public function get_submissions($group_id) {
		global $wpdb;
		$row = $wpdb->get_results($wpdb->prepare('SELECT * FROM `' . $wpdb->prefix . 'mwd_forms_submits` WHERE group_id="%d"', $group_id));
		return $row;
	}

  public function get_mailchimp_lists() {
    $api_key = get_option('mwd_api_key', '');
    $mwd_lists = (array) MWD_Library::mailchimp_curl_connect( '/lists/', 'GET', $api_key);

    $lists = array();
    if($mwd_lists['total_items'])
      foreach($mwd_lists['lists'] as $mwd_list){
        $lists[$mwd_list->id] = $mwd_list->name;
      }
    return $lists;
  }
