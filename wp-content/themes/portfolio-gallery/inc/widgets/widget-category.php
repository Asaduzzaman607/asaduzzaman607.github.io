...
class Portfolio_gallery_categ extends WP_Widget
{
	function __construct()
	{
		...
		parent::__construct(false, $name = __('Categories Posts', WDWT_LANG), $widget_ops, $control_ops);
	}
	...
	function widget($args, $instance)
	{
		...
		while ($wp_query->have_posts()) : $wp_query->the_post(); ?>
			...
			<div class="cat_widg_cont">
				...
				<div class="cat_widg">
					<?php if (has_post_thumbnail() || (Portfolio_gallery_frontend_functions::post_image_url() && $grab_image)) { ?>
						<a href="<?php the_permalink(); ?>">
	...
	{
		//Defaults
		$instance = wp_parse_args((array)$instance, array('title' => __('Categories Posts', WDWT_LANG), 'categ_id' => '0', 'post_count' => '3'));
		...
		<p><label for="<?php echo $this->get_field_id('post_count'); ?>">Number of Posts:</label><input
...
add_action('widgets_init', 'portfolio_gallery_widget_category');
function portfolio_gallery_widget_category(){
  return register_widget("Portfolio_gallery_categ");
}
?>