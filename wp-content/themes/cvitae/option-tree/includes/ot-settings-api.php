        ...
        foreach( (array) $this->get_pages( $option ) as $page ) {
          ...
          if ( ! isset( $page['hidden_page'] ) && $screen->id == $this->page_hook[$page['id']] ) {
            ...
            if ( $show_buttons ) {
              ...
              $url = admin_url() . get_current_screen()->parent_file . '?page=trendz-theme-option';
              ...
              if ( $show_buttons ) {
                ...
                echo '<form method="post" action="' . esc_url($url) . '">';
                  ...
                  echo '<button type="submit" class="option-tree-ui-button button button-secondary left reset-settings" title="' . __( 'Reset Options', 'cvitae' ) . '">' . __( 'Reset Options', 'cvitae' ) . '</button>';
