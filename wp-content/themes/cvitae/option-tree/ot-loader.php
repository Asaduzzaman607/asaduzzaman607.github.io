...
if ( class_exists( 'OT_Loader' ) && defined( 'OT_PLUGIN_MODE' ) && OT_PLUGIN_MODE == true ) {
  ...
  function ot_conflict_notice() {
    ...
    echo '<div class="error"><p>' . __( 'OptionTree is installed as a plugin and also embedded in your current theme. Please deactivate the plugin to load the theme dependent version of OptionTree, and remove this warning.', 'cvitae' ) . '</p></div>';
  ...
  class OT_Loader {
    ...
    public function load_textdomain() {
      ...
      if ( OT_PLUGIN_MODE ) {
        ...
        load_plugin_textdomain( 'cvitae', false, OT_LANG_DIR );
        
      } else {
      
        load_theme_textdomain( 'cvitae', OT_LANG_DIR );
    ...
    private function hooks() {
      ...
      add_action( 'admin_' . 'bar_menu', 'ot_register_theme_options_admin_mb', 999 );
    ...
    public function change_image_button( $translation, $text, $domain ) {
      ...
      if ( $pagenow == apply_filters( 'ot_theme_options_parent_slug', 'themes.php' ) && 'default' == $domain && 'Insert into post' == $text ) {
        ...
        $filter( 'gettext', array( $this, 'ot_change_image_button' ) );
        return apply_filters( 'ot_upload_text', __( 'Send to OptionTree', 'cvitae' ) );
